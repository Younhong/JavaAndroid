    ...
    private void checkPermissions(String[] permissions) {
        ...
        for (int i = 0; i < permissions.length; i++) {
            ...
            if (permissionCheck == PackageManager.PERMISSION_GRANTED) {
                Toast.makeText(this, currPermission + " 권한 있음.", Toast.LENGTH_LONG).show();
            } else{
                Toast.makeText(this, currPermission + " 권한 없음.", Toast.LENGTH_LONG).show();
                if (ActivityCompat.shouldShowRequestPermissionRationale(this, currPermission)) {
                    Toast.makeText(this, currPermission + " 권한 설명 필요함.", Toast.LENGTH_LONG).show();
                } else {
